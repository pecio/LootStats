...
function LootStats:LOOT_READY()
  ...
  if self.skinning or
     self.mining or
     self.herbing or
     self.engineering then
    if self.tsSeen[guid] then return end
    self.tsSeen[guid] = true
  else -- Not tradeskill, ie normal loot
    if self.seen[guid] then return end
    self.seen[guid] = true
  end
  ...
  for slot = 1, numItems do
    local lootIcon, lootName, lootQuantity, lootQuality, locked, isQuestItem, questID, isActive = GetLootSlotInfo(slot)
